...
import static org.mockito.Mockito.doNothing; // Added
...
class HandoverControllerTest {
    ...
    void createHandover_ShouldReturn201_WhenCreated() throws Exception {
        when(handoverService.create(any(HandoverDTO.class))).thenReturn(handoverDTO);

        mockMvc.perform(post("/cargoRoute/handovers/createHandover")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(handoverDTO)))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.message").value("Handover created successfully."));
    }
    ...
    void getByHandoverId_ShouldReturn200_WhenFound() throws Exception {
        when(handoverService.getById(1L)).thenReturn(responseDTO);
    ...
    void getByHandoverId_ShouldReturn404_WhenNotFound() throws Exception {
        when(handoverService.getById(99L)).thenThrow(new ResourceNotFoundException("Handover not found"));
    ...
    void getAllHandovers_ShouldReturn200_WithList() throws Exception {
        when(handoverService.getAll()).thenReturn(List.of(responseDTO));
    ...
    void getByManifestId_ShouldReturn200_WhenFound() throws Exception {
        when(handoverService.getByManifestID(10L)).thenReturn(List.of(responseDTO));
    ...
    void getByHandedBy_ShouldReturn200_WithList() throws Exception {
        String handedBy = "Warehouse Admin";
        when(handoverService.getByHandedBy(handedBy)).thenReturn(List.of(responseDTO));

        // Using URI variables in perform() ensures spaces/special characters are encoded
        mockMvc.perform(get("/cargoRoute/handovers/getHanded-by/{handedBy}", handedBy))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(1));
    }
    ...
    void updateHandover_ShouldReturn200_WhenUpdated() throws Exception {
        when(handoverService.update(eq(1L), any(HandoverDTO.class))).thenReturn(handoverDTO);

        mockMvc.perform(put("/cargoRoute/handovers/updateHandover/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(handoverDTO)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Handover updated successfully."));
    }
    ...
    void deleteHandover_ShouldReturn200_WhenDeleted() throws Exception {
        // Change: Added explicit mock for void service call
        doNothing().when(handoverService).delete(1L);

        mockMvc.perform(delete("/cargoRoute/handovers/deleteByHandoverId/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Handover deleted successfully."));
    }
}