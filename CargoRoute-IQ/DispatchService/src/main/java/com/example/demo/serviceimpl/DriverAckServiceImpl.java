...
public class DriverAckServiceImpl implements DriverAckService {
    ...
    public DriverAckDTO insert(DriverAckDTO dto) {
     ...
     if (dispatcher != null) {
         try {
             Long dispatcherId = Long.parseLong(dispatcher);
             ...
             // assignedBy is a username like "saidis", not a numeric ID
             // skip dispatcher notification safely
         }
     }
...
//    }
    ...
    private DriverAck findAck(Long id) {
        return driverAckRepository.findById(id)
                .orElseThrow(() ->
                        new ResourceNotFoundException("DriverAck not found with ID: " + id));
    ...
    private DriverAckResponseDTO buildResponse(DriverAck ack) {
        ...
        DispatchResponseDTO dispatchResponse =
                dispatchService.fetchByID(ack.getDispatch().getDispatchID());
        
        response.setDispatch(dispatchResponse);
 
        DriverDTO driver = driverService.fetchByID(ack.getDriver().getDriverID());
