...
class DispatchControllerTest {
    ...
    void createDispatch_ShouldReturn201_WhenCreated() throws Exception {
        // Change: Added mocking and verification for the service call
        when(dispatchService.insert(any(DispatchDTO.class))).thenReturn(dispatchDTO);

        mockMvc.perform(post("/cargoRoute/dispatches/createDispatch")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(dispatchDTO)))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.message").value("Dispatch created successfully."));

        verify(dispatchService).insert(any(DispatchDTO.class));
    }

    // GET DISPATCH BY LOAD ID
    @Test
    void getDispatchByLoadId_ShouldReturn200_WhenFound() throws Exception {
        when(dispatchService.findByLoadID(100L)).thenReturn(responseDTO);
    ...
    void getDispatchById_ShouldReturn200_WhenFound() throws Exception {
        when(dispatchService.fetchByID(1L)).thenReturn(responseDTO);
    ...
    void getDispatchByAssignedBy_ShouldReturn200_WithList() throws Exception {
        when(dispatchService.fetchByAssignedBy("Admin")).thenReturn(List.of(responseDTO));
    ...
    void getDispatchByStatus_ShouldReturn200_WithList() throws Exception {
        when(dispatchService.fetchByStatus(DispatchStatus.ASSIGNED)).thenReturn(List.of(responseDTO));
    ...
    void getAllDispatches_ShouldReturn200_WithList() throws Exception {
        when(dispatchService.fetchAll()).thenReturn(List.of(responseDTO));
    ...
    void updateDispatch_ShouldReturn200_WhenUpdated() throws Exception {
        // Change: Added mocking and verification for update call
        when(dispatchService.updateDispatch(eq(1L), any(DispatchDTO.class))).thenReturn(dispatchDTO);

        mockMvc.perform(put("/cargoRoute/dispatches/updateDispatch/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(dispatchDTO)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Dispatch updated successfully."));

        verify(dispatchService).updateDispatch(eq(1L), any(DispatchDTO.class));
    }
    ...
    void deleteDispatch_ShouldReturn200_WhenDeleted() throws Exception {
        mockMvc.perform(delete("/cargoRoute/dispatches/deleteDispatch/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Dispatch deleted successfully."));

        verify(dispatchService).delete(1L);
    }
}